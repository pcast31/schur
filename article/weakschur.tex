...
\begin{definition}
Let \( (n,k,b) \in (\mathbb{N}^*)^3\), Let \((A_1,....,A_k)\) a partition of  \([\![1, n]\!]\). This partition is said to be a (n,k,b)-weakly-sum-free template when:
\\\\
...
\\
\[if \quad x+y>b+2(n-b),\quad x+y-2(n-b)\notin A_k\]
\\
\underline{For the others subsets:}\quad \(\forall i \in [\![0,k-1]\!], \forall(x,y) \in A_i^2\)
\\
\[
(Id+(n-b)\mathbf{1}_{ [\![0,b]\!]})(x+y ~\text{mod}~ (n-b)) \notin A_i
\]
\end{definition}
\\\\
\begin{definition}
Let \( (k,b) \in (\mathbb{N}^*)^2\), if there exist n such as there exist a partition of k subsets of \([\![1, n]\!]\) which is a (n,k,b)-weakly-sum-free template, we note: 
\\\\\(WS_b^+=\max \{n\in \mathbb{N}^*\)/ there exists a partition of k-subsets of \([\![1, n]\!]\) which is a (n,k,b)-weakly-sum-free template \}
\\\\
If this n does not exist, we set \(WS_b^+= 0\)
\end{definition}
...
\begin{theorem}
Let \((n,k,b) \in (\mathbb{N}*)^3\), let \( (m,p) \in (\mathbb{N}^*)^2\). If there exists a partition of p sum-free subsets of \([\![1,m]\!]\) and a partition of k subsets \((A_1,....,A_k)\) of \([\![1, n]\!]\) which is a (n,k,b)-weakly-sum-free template, then there exists a partition of (k+p-1) weakly sum-free subsets of \([\![1, b+c+m \times n]\!]\) with c=\(min A_n\bigcap [\![b+1, n]\!]\)-b-1
\end{theorem}
