...
If there is no confusion on the \(a\) and \(b\) to use, \(\pi_{a, b}\) is denoted by \(\pi\). Notice that for all
\(x \in \mathbb{Z}, \pi(x) = x \mod a\) and for all \(x \in [\![b + 1, a + b]\!], b + 1 \leqslant \pi(x) \leqslant a + b\).

\begin{sloppypar}
\(\pi\) is the projection on the first line mentioned in the intuitive explanation. The following four propositions are
calculative properties on \(\pi\) reflecting the behaviour of an element's column number in \hyperref[SchemaWS]{Figure \ref{SchemaWS}} and
that we will use later when we introduce WSF-templates.
\end{sloppypar}
...
After defining the function related to the column number of each element in \hyperref[SchemaWS]{Figure \ref{SchemaWS}},
...
\hyperref[ConstructionWSF]{Subsection 3.3}.
...
This partition is said to be a \(b\)-weakly-sum-free template (\(b\)-WSF-template) with width \(a\) and \(n\) colors when :
...
We now introduce the number \(\WS^+(n)\) that plays the same role as \(\S^+(n)\) for SF-templates.
However, WSF-templates being more sophisticated than SF-templates, the definition of \(\WS^+(n)\)
...
Here, we consider \(\pi\) defined in \hyperref[DefinitionWS+]{subsection 3.2} as an application defined on \([\![b + 1, p a + b]\!]\). 
...
use this sophistications given that we could not even find good WSF-templates with five colors using a computer (here good
...
\hyperref[Theorem3.22]{Theorem 3.22} yields the following corollary.
...
\((a, b)\) (in the lexicographic order). As we will see, this way of computing lower bounds is not necessarily the best
we can do. In the following, we give the strongest inequalities we have so far and further insight about how they were produced.
...
\hyperref[WS(n+1)]{(\ref{WS(n+1)})} and \hyperref[WS(n+2)]{(\ref{WS(n+2)})} were already found by Rowley, they are detailed in \cite{RowleyWS}. \hyperref[WS(n+3)]{(\ref{WS(n+3)})} is optimal 
and was found with a SAT solver. It uses the first sophistication explained in the \hyperref[ConstructionWSF]{Subsection 3.3} in order to add 
the last number in the first color. The \hyperref[WSF-templates]{corresponding template} can be found in the appendix.
As for \hyperref[WS(n+4)]{(\ref{WS(n+4)})}, it was obtained by combining an optimal SF-template with width 33 with a WSF-template with width 4.
...
five colors which start with a good \(\WS(4)\) partition and we assumed that the special color was the last by order of 
...
Like in Subsection 2.3 \textbf{LABEL NECESSAIRE}, we compute the lower bounds given by \hyperref[WS(n+1)]{(\ref{WS(n+1)})}, 
\hyperref[WS(n+2)]{(\ref{WS(n+2)})} and \hyperref[WS(n+3)]{(\ref{WS(n+3)})} for \( n \in [\![8,15]\!] \). The best lower bound
...
With \( S(9) \geqslant 17\,803 \), we found a new lower bound for \(\WS(10)\) using \hyperref[WS(n+1)]{(\ref{WS(n+1)})}.
Moreover, \hyperref[WS(n+3]{(\ref{WS(n+3)})} gives new lower bounds for \(\WS(9)\) and \(WS(14)\).
...
obtained with WSF-templates (including \hyperref[WS(n+1)]{(\ref{WS(n+1)})} and \hyperref[WS(n+2]{(\ref{WS(n+2)})}).
