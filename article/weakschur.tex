...
calculative properties on \(\pi\) reflecting the behaviour of an element's column number in Figure \ref{SchemaWS} and
...
After defining the function related to the column number of each element in Figure \ref{SchemaWS},
...
\(p\) and \(k+1\) colors and a \(b\)-WSF-template with width \(a\) and \(n\) colors, then there exists a \(pb\)-WSF-template 
...
(\ref{WS(n+1)}) and (\ref{WS(n+2)}) were already found by Rowley, they are detailed in \cite{RowleyWS}. (\ref{WS(n+3)}) is optimal 
...
As for (\ref{WS(n+4)}), it was obtained by combining an optimal SF-template with width 33 with a WSF-template with width 4.
...
Like in Subsection 2.3 \textbf{LABEL NECESSAIRE}, we compute the lower bounds given by (\ref{WS(n+1)}), 
(\ref{WS(n+2)}) and (\ref{WS(n+3)}) for \( n \in [\![8,15]\!] \). The best lower bound
...
Moreover, (\ref{WS(n+3)}) gives new lower bounds for \(\WS(9)\) and \(WS(14)\).
...
obtained with WSF-templates (including (\ref{WS(n+1)}) and (\ref{WS(n+2)}).
