...
Hanson's construction and with an S-template which is not included in Abbott and Hanson's construction. In both cases, 
...
\begin{proof}[\textsc{Proof.}]
	...
	\(u \in [\![1,q]\!]\). Obviously, this decomposition unique. Coefficients \(\alpha\) and \(u\) may be interpreted as the row index of \(x\) and 
	as line and column coordinates of \(x\) in the template. A new coloring is defined as:
	\[
	\begin{array}{c c c l}
...
We produced S-templates using a SAT solver \cite{Lingeling2017}, hence providing lower bound on \(S^+\) and inequalities 
of the type \(S(n+k) \geqslant a S(n) + b\). We sought templates providing us with the largest value of 
\((a, b)\) (in the lexicographic order). In order to reduce the search space when the number of colors exceeded 
five we 
...
(\ref{S(n+5)}) are listed in Appendix \ref{S-templates}.
...
Appendix \ref{S-templates}). Although we could not find a better S-template with seven colors, inequality (\ref{S(n+6)}) can definitely be 
