...
We now assume \(h(x) > k\) (and also \(h(x+y) > k\) otherwise the expected result is trivial). 
Then \(h(x) = k + g(\alpha) = k + g(\beta) = h(y)\), hence \(g(alpha) = g(\beta\). We have the two same cases as before.
