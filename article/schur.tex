...
of Figure 1 not as a combination of two blocs anymore but as a whole, single construction. We thereby define a S-template as
...
Here \(n\) is the "special" color: the numbers colored in the special color has less constraints than the other colors. However, please note that \(n\) 
...
\qquad We start by reminding the explicite construction of a sum-free partition with the use of a S-template and another sum free partition. We apply this contruction stated by Rowley in the context of Ramsey numbers to the Schur numbers.
...
We now proceed to prove \hyperref[Theorem2.4]{Theorem 2.4}
...
\begin{proof}[\textsc{Proof.}]
	...
	\(u \in [\![1,q]\!]\). This decomposition is of course unique. \(\alpha\)  can be interpreted as the row number of \(x\) and \(u\) as the collumn number of \(x\). A new coloring is define as follows:
	\[
	\begin{array}{c c c l}
...
The following proposition may improve the additive constant of \hyperref[Corollary2.5]{Corollary 2.5}
...
In the following, we give the strongest inequalities we have so far and further insight about how they were produced.
...
\par
Here are the best inequalities on Schur numbers so far (the \hyperref[S-templates]{templates} corresponding to 
...
\begin{align}
	...
	S(n + 6) \geqslant 1\,140 S(n) + 528 \label{S(n+6)}
\end{align}


\hyperref[S(n+1)]{(\ref{S(n+1)})} comes from  Schur's original article\cite{Schur1917}. \hyperref[S(n+2)]{(\ref{S(n+2)})} is due to
Abott \cite{AbbottHanson}
and \hyperref[S(n+3)]{(\ref{S(n+3)})} to Rowley \cite{RowleyRamsey}. \hyperref[S(n+4)]{(\ref{S(n+4)})}, \hyperref[S(n+5)]{(\ref{S(n+5)})} and \hyperref[S(n+6)]{(\ref{S(n+6)})} are our result.

\par
\hyperref[S(n+1)]{(\ref{S(n+1)})}, \hyperref[S(n+2)]{(\ref{S(n+2)})} and \hyperref[S(n+3)]{(\ref{S(n+3)})} are optimal. \hyperref[S(n+4)]{(\ref{S(n+4)})} is optimal among symmetric S-templates whose special color is 
the last in the order of apparition (and with a multiplicative factor less than or equal to 118). \hyperref[S(n+5)]{(\ref{S(n+5)})} is most 
likely not optimal but should not be too far from the optimum. 
Finally, \hyperref[S(n+6)]{(\ref{S(n+6)})} is obtained by combining a S-template with width 380 and one with width three. 
Although we could not find a better S-template with seven colors, \hyperref[S(n+6)]{(\ref{S(n+6)})} is definitely very far from the 
