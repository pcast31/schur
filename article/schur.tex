...
\begin{theorem}
\label{thm:Stemp}
	Let \((p,k), (q,n) \in (\mathbb{N}^*)^2\). If there are a \(S\)-template with width \(q\) and \(n+1\) colors 
...
(\ref{S(n+6)}) was found by extending into an S-template the Schur 6 partition used in \cite{rowley2021improved} (owing to the 
