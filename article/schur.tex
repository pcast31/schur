...
\begin{theorem}
	Let \((p,k), (q,n) \in (\mathbb{N}^*)^2\). If there exists a \(SF\)-template with width \(p\) and \(k+1\) colors 
...
\begin{proof}[\textsc{Proof.}]
Denote by \(f\) the coloring associated to the \(SF\)-template with width \(p\) and \(g\) the one associated 
to the sum-free partition of \([\![1,q]\!]\); where \(f : [\![1,p]\!] \longrightarrow [\![1,k+1]\!]\) and 
\(g : [\![1,q]\!] \longrightarrow [\![1,n]\!]\).

\par
NB: In the following 3 predicates, the conditions \(x + y \leqslant p\)  and \(x + y \leqslant q\) are omitted for readability.
\par
The sum-free condition is expressed as
\[\forall (x,y) \in [\![1,p]\!]^2, f(x) = f(y) \Longrightarrow f(x+y) \neq f(x)\]
\[\forall (x,y) \in [\![1,q]\!]^2, g(x) = g(y) \Longrightarrow g(x+y) \neq g(x)\]
...
\noindent \underline{\textbf{Case 2:} \(h(x) \geqslant k + 1\)}
\par
\(h(x) = k + g(\alpha) = 
...
\item \begin{sloppypar}
	If \(u + v > p\), write \(w = u + v - p \in [\![1, p]\!]\). Then \(x + y = (\alpha + \beta - 1) p + w\). Assume that 
...
\begin{proposition}
Let \((p, k) \in \mathbb{N}^*)^2\) and let \(f\) be a coloring associated to a SF-template with width \(p\) and \(k+1\) 
colors. Let \(b \in \mathbb{N}\) (\(b = m_{k+1} - 1\) works) and assume there there exists a coloring \(g\) of 
...
\begin{theorem}
Let \((p,k), (q,n) \in (\mathbb{N}^*)^2\). If there exists a SF-template with width \(p\) and \(k+1\) colors,
and a SF-template with width \(q\) and \(n\) colors, then there exists SF-template with width \(pq\) and \((n+k)\) 
colors.
\end{theorem}
