...
However, please note that \(n\) is not necessarily the last color by order of appearance.
...
\label{Subsection2.3}
We now give the inequalities correspondng to the current  best S-templates (to our knowledge).
...
likely be further improved but the improvement proabably will not be substantial. Finally, inequality (\ref{S(n+6)}) is obtained 
by combining a S-template with width 380 and one with width three. Although we could not find a better S-template with seven 
colors, inequality (\ref{S(n+6)}) can definitely be improved by a wide margin. One may try to seek better templates by 
constraining less the search space and by using Monte-Carlo methods, as in \cite{Bouzy2015AnAP}. This could be the subject 
of a future work.
