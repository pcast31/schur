...
\begin{theorem}
	...
	-1]\!]\) into \(n+k\) sum-free subsets. \(m_{n+1}\) is the minimum of the special subset in the \(SF\)-template.
\end{theorem}
...
\(q = 4\), \(n = 2\) and \(k = 2\) showing that \(S(2 + 2) \geqslant S(2) (2 S(2) + 1) + S(2)\), both with Abbott and 
...
\begin{proof}[\textsc{Proof.}]
	...
	\par
	NB: In the following 3 predicates, the conditions \(x + y \leqslant q\)  and \(x + y \leqslant p\) are omitted for readability.
	\par
	...
	For \(x \in [\![1,pq+m_{n+1}-1]\!]\), write \(x = (\alpha - 1) + u\) for some integers \(\alpha \in \mathbb{Z}\) and 
	...
	\begin{array}{c c c l}
		...
		\left\{ \begin{array}{l l}
			...
			k + g(\alpha) & \text{if}~f(u) = n + 1 \\
		\end{array} \right.
	\end{array}
	\]
	
	\(h\) is well-defined since, by definition of \(m_{n+1}\), \(\forall x \in [\![p q + 1, p q + m_{n + 1} - 1 ]\!], f(u) 
	\leqslant k\) and therefore \(\forall x \in [\![1,pq+m_{n+1}-1]\!], f(u) = n + 1 \implies \alpha \in [\![1, p]\!]\).
	
	\par
	We now prove that \(h\) is a sum-free coloring. Let \(x,y \in [\![1,pq + m_{n+1}-1]\!]\) such that \(h(x) = h(y)\) 
	and \(x+y \leqslant pq+m_{n+1}-1\). We claim that \(h(x+y) \neq h(x)\). Write \(x = (\alpha - 1) q + u\) and 
	\(y = (\beta - 1) q + v\) where \(\alpha, \beta \in \mathbb{Z}\) and \(u, v \in [\![1,q]\!]\). Distinguish between 
	two cases according to the value of \(h(x)\). \\
	
	\noindent \underline{\textbf{Case 1:} \(h(x) \leqslant k\)}
	\par
	Assume that \(h(x+y) \leqslant k\), otherwise \(h(x + y) \neq h(x)\) obviously holds. By definition of \(h\) 
	and given that \(h(u) = h(v)\), \(f(u) = f(v)\). Distinguish between two cases according to the value of \(x + y\).
	
	\begin{itemize}
	\item If \(u + v > q\), write \(w = u + v - q \in [\![1, q]\!]\). Then \(x + y = (\alpha + \beta - 1) q + w\). By definition, 
		\(h(x + y) = f(w)\). Given that \(f(u) = f(v) \leqslant n\), the additionnal constraint on \(f\) implies \(f(w) 
		\neq f(u)\), that is \(h(x + y) \neq h(x)\).
	\item If \(u + v \leqslant p\), write \(w = u + v \in [\![1, q]\!]\). Then \(x+y = (\alpha + \beta- 2) q + w\). By definition, 
...
\begin{proposition}
	...
	colors. Let \(b \in \mathbb{N}\) (\(b = m_{n+1} - 1\) works) and assume there there exists a coloring \(g\) of 
	\([\![1, b]\!]\) with \(n+1\) colors such that
	
	\begin{itemize}
		\item \(\forall (x, y) \in [\![1, q]\!]^2,(f(x) = f(y) \text{ and } (x + y) \mod q \leqslant b) 
		\implies g((x + y) \mod q) \neq f(x)\)
		\item \(\forall (x, y) \in [\![1, q]\!] \times  [\![1, b]\!], (f(x) = g(y) \text{ and } x + y \leqslant b) 
		\implies g(x + y) \neq f(x)\)
	\end{itemize}
...
\begin{theorem}
	Let \((p,k), (q,n) \in (\mathbb{N}^*)^2\). If there exists a SF-template with width \(q\) and \(n+1\) colors,
	and a SF-template with width \(p\) and \(k\) colors, then there exists SF-template with width \(pq\) and \((n+k)\) 
	colors.
	\end{theorem}
...
We exhibited SF-templates using a SAT solver, hence providing lower bound on \(S^+\) and inequalities 
...
Although we could not find a better SF-template with 7 colors, the last inequality is definitely very far from the 
