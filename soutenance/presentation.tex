...
\begin{frame}
La borne inférieure établie par I. Schur est plutôt grossière :
\[\forall n \in \mathbb{N}^*, S(n+1) \geqslant 3S(n) + 1 \Longrightarrow \forall n \in \mathbb{N}^*, S(n) \geqslant \frac{3^n - 1}{2}
\]
Une première piste pour améliorer cette borne est proposée par H. L. Abbott et D. Hanson en 1972. Ils prouvent :
\[
\forall (m,n) \in (\mathbb{N}^*)^2, S(n+m) \geqslant S(n) \left( 2S(m) + 1 \right) + S(m)
\]
...
\item On conserve l'extension verticale est horizontale de "structures" intéressantes.
\item Cependant, on ne considère plus la première ligne comme un assemblage mais comme une structure en elle même.
\item On applique l'extension verticale à une structure plus générale : les SF-templates.
\end{itemize}
